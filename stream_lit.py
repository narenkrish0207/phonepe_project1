    ...
    if sub_tab == "Transaction":
        ...
        if not df_total.empty:
            ...
            st.metric(
                ...
                f"{int(total_transactions):,}" if total_transactions is not None else "NILL"
            )

            st.metric(
                "Total Insurance Amount (₹)",
                f"{int(total_amount):,}" if total_amount is not None else "NILL"
            )
        else:
            st.metric("Total Insurance Transactions", "NILL")
            st.metric("Total Insurance Amount (₹)","NILL")
        tab1, tab2, tab3 = st.tabs(["States", "Districts", "Pincodes"])
